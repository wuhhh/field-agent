...
class OperationsExecutorService extends Component
{
	...
	private function executeCreateOperation(array $operation, array $result, array $createdFields = [], array $createdEntryTypes = []): array
	{
		...
		switch ($operation['target']) {
			case 'field':
				...
				// Matrix field tracking is now handled internally by MatrixField class
				...
				if ($field) {
					...
					if (!Craft::$app->getFields()->saveField($field)) {
						throw new Exception("Failed to save field '{$field->name}' ({$field->handle})");
					}
					...
					if ($field instanceof \craft\fields\Matrix) {
						...
						$blockCount = count($entryTypes);
						if ($blockCount > 0) {
							$result['message'] .= " (with $blockCount block types)";
						}
					}
				} else {
	...
	private function modifyField(array $operation, array $result): array
	{
		...
		foreach ($actions as $action) {
			if ($action['action'] === 'updateField' && isset($action['updates'])) {
				...
				if ($entryTypeConfig) {
					$plugin = Plugin::getInstance();
					$registry = $plugin->fieldRegistryService;
					$matrixFieldDefinition = $registry->getField('matrix');

					if ($matrixFieldDefinition) {
						$matrixField = new \craftcms\fieldagent\fieldTypes\MatrixField();
						if ($matrixField->addMatrixEntryType($field, $entryTypeConfig)) {
				...
				if ($entryTypeHandle) {
					...
					if ($matrixFieldDefinition) {
						$matrixField = new \craftcms\fieldagent\fieldTypes\MatrixField();
						if ($matrixField->removeMatrixEntryType($field, $entryTypeHandle)) {
				...
				if ($entryTypeHandle && $entryTypeUpdates) {
					...
					if ($matrixFieldDefinition) {
						$matrixField = new \craftcms\fieldagent\fieldTypes\MatrixField();
						if ($matrixField->modifyMatrixEntryType($field, $entryTypeHandle, $entryTypeUpdates)) {
							$changeDesc = [];
		...
		if (!empty($modifications)) {
			if (Craft::$app->getFields()->saveField($field)) {
				...
				throw new Exception("Failed to save field '{$targetId}' after modifications");
			}
		} else {
			$result['success'] = true;
	...
	private function deleteField(string $fieldHandle, bool $force, bool $cascade, array $result): array
	{
		...
		if (!Craft::$app->getFields()->deleteField($field)) {
			$result['success'] = false;
			$result['message'] = "Failed to delete field '{$fieldHandle}'";
			return $result;
		}
